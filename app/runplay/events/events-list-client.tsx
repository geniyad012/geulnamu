...
export default function RunplayEventsListClient() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "ongoing":
        return <span className="px-3 py-1 bg-yellow-100 text-yellow-700 text-xs font-semibold rounded-full">진행중</span>
      case "completed":
  ...
  const renderEventCard = (event: Event) => (
    <Link key={event.id} href={`/runplay/events/${event.id}`}>
        ...
        <div className="p-4">
          ...
          <div className="flex items-center justify-between mt-3 gap-2">
            ...
            <button
              onClick={(e) => {
                e.preventDefault()
                window.location.href = `/runplay/events/${event.id}`
              }}
              type="button"
              className="px-2.5 py-1 bg-yellow-500 hover:bg-yellow-600 text-black text-xs font-semibold rounded-md transition-colors whitespace-nowrap"
            >
              신청하기
            </button>
          </div>
      ...
      <div className="max-w-[430px] mx-auto px-4 py-6">
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-gray-500 text-sm mb-4">잠시 후 다시 시도해주세요</p>
